...
- **15/05/2026 (sera)** - 🎯 **6 richieste utente — Tutte implementate**:
  ...
  - **💰 (4) Fatture clienti — 3 tipi**: nuovo `action=invoice` su `/api/demos/{id}` che riusa la logica del preventivo ma genera PDF "FATTURA" con numerazione progressiva annuale `FAT-{YYYY}-NNNN` (contatore in `user_settings.invoice_counter_{year}`), scadenza pagamento a 30 giorni, salvataggio in collection `invoices`. UI: nuova Card emerald "Emetti fattura cliente" nella pagina Clients (sotto Preventivi/WhatsApp) con 2 bottoni: "Scarica Fattura PDF" + "Invia Fattura via Email". Funziona con TUTTI e 3 i regimi (Con P.IVA / Forfettario / Prestazione occasionale) usando lo stesso selettore della tipologia preventivo.
  - **🎨 (2) Color picker libero per titolo sito**: nuovo blocco "Colore Titolo & Hero Info" in StyleEditor con 2 color picker HEX (titolo + hero info bar). Salvati in `content.title_color` / `content.hero_bar_color`. Frontend DemoPreview applica `style={{color: content.title_color}}` agli `<h1>` hero. Validazione backend: HEX `#RGB` o `#RRGGBB`.
  - **📍 (3) Hero Info Bar evidenziata con colore**: la barra indirizzo/telefono/rating sotto la foto hero ora supporta uno sfondo custom HEX. Calcolo automatico contrasto: testo nero/bianco scelto in base alla luminanza WCAG del colore scelto. Shadow aggiuntiva per maggiore evidenza.
  - **✨ (6) Animazioni reveal-on-scroll + decorazioni SVG flottanti**: IntersectionObserver applica `.wf-in` alle sezioni quando entrano in viewport (fade-up 700ms). 3 SVG decorazioni floating (blob gradient + dots animati) tra le sezioni con keyframes `wfFloat` e `wfPulse`. Rispetta `prefers-reduced-motion`.
  - **⚙️ Mirror Preview**: `editor-data` Vercel ora restituisce anche `title_color` e `hero_bar_color`. TEMPLATE_FIELDS coerenti tra Vercel e FastAPI. Settings layout-default disponibile su entrambi (Vercel + FastAPI).

- **15/05/2026 (mattina)** - 🐛 **Fix Salvataggio/Applicazione Template**:
